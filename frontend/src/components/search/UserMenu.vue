...
<template>
  <div class="flex flex-col gap-2 w-[260px]">
    ...
    <span v-if="appState.payment_supported">
      <button @click="djangoClient.post(`/org/consider_buying/`).then(
        () => { _window.location.assign(appState.payment_plans_url) })"
        class="text-xs text-gray-500 bg-blue-500 text-white hover:bg-blue-700 py-2 px-4 rounded-lg w-full">
        Get more credits
      </button>
    </span>

    <span v-if="!appState.payment_supported" class="text-xs text-gray-500">
    ...
    <LlmSelect :placeholder="$t('UserMenu.llm-select-placeholder')"
      v-model="appState.user.preferences.default_small_llm" :show_default_option="true"
    ...
    <LlmSelect :placeholder="$t('UserMenu.llm-select-placeholder')"
      v-model="appState.user.preferences.default_large_llm" :show_default_option="true"
      @update:modelValue="appState.commit_user_preferences()">
    </LlmSelect>
    ...
    <div v-if="show_change_password" class="flex flex-col gap-1">
      <input ref="old_password" type="password" :placeholder="$t('UserMenu.old-password')"
        class="w-full rounded-md border-0 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-400 sm:text-sm sm:leading-6" />
      <input ref="new_password" type="password" :placeholder="$t('UserMenu.new-password')"
        class="w-full rounded-md border-0 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-400 sm:text-sm sm:leading-6" />
      <input ref="new_password_repeat" type="password" :placeholder="$t('UserMenu.repeat-new-password')"
        @keyup.enter="change_password($refs.old_password.value, $refs.new_password.value, $refs.new_password_repeat.value)"
      ...
      <button
        @click="change_password($refs.old_password.value, $refs.new_password.value, $refs.new_password_repeat.value)"
    ...
    <div class="flex flex-row items-center gap-1" v-if="appState.logged_in && appState.user?.is_staff">
      <a :href="`org/admin/`" title="Manage Datasets" class="w-8 rounded p-2 text-sm text-gray-500 hover:bg-gray-100">
        <CircleStackIcon></CircleStackIcon>
      </a>
      <button @click="appState.toggle_dev_mode(); $emit('hide');"
