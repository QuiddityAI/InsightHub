  ...
  data() {
    return {
      ...
      available_task_types: [
        { task_type: 'quick_search', title: 'Quick Search' },
        { task_type: 'topic_overview', title: 'Topic Overview' },
        { task_type: 'question', title: 'Question' },
        { task_type: 'high_precision_search', title: 'High Precision Search' },
        { task_type: 'custom_search', title: 'Custom Search' },
      ],
...
<template>
  <div class="px-12 pt-10 pb-10 pointer-events-auto rounded-md bg-white shadow-sm">
    ...
    <!-- Task Type Selection -->
    <p v-if="!selected_task_type" class="mb-4 text-lg text-gray-500">
      Select what you want to do:</p>
    <div class="flex flex-row items-center justify-between gap-8 text-gray-600">
    ...
    <div v-if="selected_task_type === 'quick_search'" class="mt-10">
      <!-- Database Selection -->
          ...
          <input
            ...
            :placeholder="
              appState.settings.search.search_type == 'external_input'
                ? `Describe what ${appState.settings.search.dataset_ids.length ? appState.datasets[appState.settings.search.dataset_ids[0]]?.schema.entity_name || '' : ''} you want to find`
                : 'But more like this:'
            "
            class="h-full w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-400 sm:text-sm sm:leading-6" />
        </div>
