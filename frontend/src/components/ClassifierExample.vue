...
<div class="rounded px-3 py-2" :class="{'bg-green-100/50': is_positive, 'bg-red-100/50': !is_positive}">
  <p class="text-sm font-medium leading-6 text-gray-900" v-html="appState.classifier_example_rendering.title(item)"></p>
  <p class="truncate text-xs leading-5 text-gray-500" v-html="appState.classifier_example_rendering.subtitle(item)"></p>
  <img v-if="appState.classifier_example_rendering.image(item)" class="h-24" :src="appState.classifier_example_rendering.image(item)">
