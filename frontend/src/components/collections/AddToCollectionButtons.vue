...
<template>
  <div class="flex flex-row h-7">
      <button class="flex flex-row items-center gap-2 border border-gray-300 rounded-l-md px-3 text-sm text-gray-500 hover:text-blue-500 hover:bg-gray-100"
        @click="$emit('addToCollection', this.appStateStore.last_used_collection_id, this.appStateStore.last_used_collection_class, true)"
    ...
    <OverlayPanel ref="select_collection_overlay">
      <div class="flex flex-row gap-3">
        <select
          v-model="this.appStateStore.last_used_collection_id"
        ...
        <select
          v-if="this.appStateStore.last_used_collection_id !== null"
          v-model="this.appStateStore.last_used_collection_class"
          ...
          <option
            v-for="class_details in appState.collections.find((e) => e.id == this.appStateStore.last_used_collection_id).actual_classes"
        ...
        <button
          ...
          @click="
            $emit('addToCollection', this.appStateStore.last_used_collection_id, this.appStateStore.last_used_collection_class, true);
        ...
        <button
          ...
          @click="
            $emit('addToCollection', this.appStateStore.last_used_collection_id, this.appStateStore.last_used_collection_class, false);
            $refs.select_collection_overlay.hide()
          "
        ...
        <button
          v-if="multiple_items"
          @click="
            $emit('removeFromCollection', this.appStateStore.last_used_collection_id, this.appStateStore.last_used_collection_class);
            $refs.select_collection_overlay.hide()
          "
