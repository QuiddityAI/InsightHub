  ...
  methods: {
    ...
    setupWebGl() {
      ...
      function update(currentTimeInMs) {
        ...
        if (
          ...
          that.currentPositionsX.length != that.per_point.x.length
        )
          return
    ...
    updateGeometry() {
      ...
      this.currentPositionsX = ensureLength(
        ...
        pointCount,
        pointCount > 0 ? math.mean(this.per_point.x) : 0.0,
        true
      )
      this.currentPositionsY = ensureLength(
        this.currentPositionsY,
        pointCount,
        pointCount > 0 ? math.mean(this.per_point.y) : 0.0,
        true
      )
    ...
    updateMeshesQuads() {
      ...
      const shadowGeometry = new Geometry(this.glContext, {
        ...
        pointSize: {
          instanced: 1,
          size: 1,
          data: new Float32Array(this.per_point.size.slice(0, pointCount)),
        },
        pointVisibility: {
          instanced: 1,
          size: 1,
      ...
      const pointsGeometry = new Geometry(this.glContext, {
        ...
        clusterId: {
          instanced: 1,
          size: 1,
          data: new Float32Array(this.per_point.cluster_id.slice(0, pointCount)),
        },
        pointSize: {
          instanced: 1,
          size: 1,
        ...
        opacity: {
          instanced: 1,
          size: 1,
          data: new Float32Array(this.per_point.opacity.slice(0, pointCount)),
        },
        secondary_hue: {
          instanced: 1,
          size: 1,
          data: new Float32Array(this.per_point.secondary_hue.slice(0, pointCount)),
        },
        secondary_sat: {
          instanced: 1,
          size: 1,
          data: new Float32Array(this.per_point.secondary_sat.slice(0, pointCount)),
        },
        secondary_val: {
          instanced: 1,
          size: 1,
          data: new Float32Array(this.per_point.secondary_val.slice(0, pointCount)),
        },
        secondary_opacity: {
          instanced: 1,
          size: 1,
          data: new Float32Array(this.per_point.secondary_opacity.slice(0, pointCount)),
        },
        pointVisibility: {
          instanced: 1,
          size: 1,
    ...
    onMouseUp(event) {
      ...
      if (event.shiftKey) {
        if (this.appStateStore.selected_point_indexes.includes(this.hoveredPointIdx)) {
          this.appStateStore.selected_point_indexes =
            this.appStateStore.selected_point_indexes.filter((x) => x !== this.hoveredPointIdx)
        } else {
    ...
    executeLassoSelection(mode = "replace") {
      ...
      if (mode == "replace") {
        ...
        this.appStateStore.selected_point_indexes = [
          ...new Set([...this.appStateStore.selected_point_indexes, ...selectedPointIndexes]),
        ]
      } else if (mode == "remove") {
        this.appStateStore.selected_point_indexes =
          this.appStateStore.selected_point_indexes.filter(
...
<template>
  <div>
    ...
    <div
      ...
      :class="{
        'cursor-crosshair': appStateStore.selected_map_tool === 'lasso',
    ...
    <div
      v-for="cluster_label in clusterData"
      class="fixed"
      :style="{
      ...
      <button
        v-if="
          appStateStore.selected_cluster_id === null ||
          cluster_label.id === appStateStore.selected_cluster_id
        "
    ...
    <div
      ...
      <div
        v-if="per_point.text_data.length > hoveredPointIdx && hover_label_rendering"
        ...
        <img
          v-if="hover_label_rendering.image(per_point.text_data[hoveredPointIdx])"
          :src="hover_label_rendering.image(per_point.text_data[hoveredPointIdx])"
          class="h-24" />
      </div>
      <div
        v-if="per_point.text_data.length <= hoveredPointIdx || !hover_label_rendering"
