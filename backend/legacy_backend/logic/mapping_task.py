...
def generate_map(map_id: str, ignore_cache: bool):
    ...
    if similar_map:
        same_search_and_vectorize_settings = vectorize_stage_params_hash == get_vectorize_stage_hash(
            similar_map["last_parameters"]
        )
        same_search_vec_and_projection_settings = projection_stage_params_hash == get_projection_stage_hash(
            similar_map["last_parameters"]
        )
    ...
    if search_phase_can_be_resused:
        ...
        sorted_ids, items_by_dataset = reuse_search_phase(
            map_data, similar_map, params, datasets, items_by_dataset, timings
        )
        timings.log("reused search stage results")
    else:
        sorted_ids, items_by_dataset, thumbnail_atlas_thread = search_phase(
            map_data, params, datasets, items_by_dataset, timings
        )
        if not sorted_ids:
    ...
    no_vectorization_needed = (
        search_phase_can_be_resused and projection_phase_can_be_reused and all_map_vectors_present
    )
    ...
    if projection_phase_can_be_reused:
        ...
        raw_projections, final_positions = projection_phase(
            map_data, params, datasets, items_by_dataset, sorted_ids, timings
        )

    # ----------------- Clusterize and Render Phase -----------------

    clusterize_and_render_phase(
        map_data, params, datasets, items_by_dataset, sorted_ids, raw_projections, final_positions, timings
    )
...
def search_phase(
    map_data: dict, params: DotDict, datasets: dict, items_by_dataset: dict, timings: Timings
) -> tuple[list[tuple[str, str]], dict, Thread | None]:
...
def thumbnail_phase(datasets, items_by_dataset, map_data, params, sorted_ids, timings: Timings):
    ...
    for ds_id, dataset in datasets.items():
        ...
        if dataset.schema.object_fields[dataset.schema.thumbnail_image].generator:
            ...
            pipeline_steps, required_fields, _ = get_pipeline_steps(
                dataset, only_fields=[dataset.schema.thumbnail_image]
            )
...
def reuse_search_phase(
    map_data: dict, similar_map: dict, params: DotDict, datasets: dict, items_by_dataset: dict, timings: Timings
    ...
    map_data["results"]["per_point_data"]["thumbnail_aspect_ratios"] = deepcopy(
        similar_map["results"].get("per_point_data", {}).get("thumbnail_aspect_ratios")
    )
...
def add_missing_vectors(
    ...
    params: DotDict,
    datasets: dict,
    items_by_dataset: dict,
    similar_map: dict | None,
    vectorize_stage_params_hash: str,
    all_map_vectors_present: bool,
    timings: Timings,
):
...
def projection_phase(
    ...
    if projection_parameters.x_axis.type == "number_field":
        ...
        cartesian_positions[:, 0] = [
            len(v)
            for v in get_field_from_all_items(items_by_dataset, sorted_ids, projection_parameters.x_axis.parameter, [])
        ]
    ...
    if projection_parameters.y_axis.type == "number_field":
        ...
        cartesian_positions[:, 1] = [
            len(v)
            for v in get_field_from_all_items(items_by_dataset, sorted_ids, projection_parameters.y_axis.parameter, [])
        ]
    ...
    def transform_coordinate_system_and_write_to_map():
        ...
        if projection_parameters.use_polar_coordinates:
            final_positions = polar_to_cartesian(
                1 - normalize_array(final_positions[:, 0]), normalize_array(final_positions[:, 1]) * np.pi * 2
            )
    ...
    if reduced_dimensions_required:
        ...
        if cluster_hints:
            ...
            vectors = np.asarray(
                get_field_from_all_items(items_by_dataset, sorted_ids, map_vector_field, dummy_vector)
            )
            ...
            for i, ds_and_item_id in enumerate(sorted_ids):
                ...
                vectors[i][: len(cluster_hints)] = [
                    1 if hint.lower() in title_abstract.lower() else 0 for hint in cluster_hints
                ]
...
def reuse_projection_phase(map_data: dict, similar_map: dict) -> tuple[np.ndarray, np.ndarray]:
    ...
    map_data["results"]["last_position_update"] = time.time()
    final_positions = np.column_stack(
        [map_data["results"]["per_point_data"]["positions_x"], map_data["results"]["per_point_data"]["positions_y"]]
    )
    map_data["progress"]["embeddings_available"] = True
    return raw_projections, final_positions


def clusterize_and_render_phase(
    map_data: dict,
    params: DotDict,
    datasets: dict,
    items_by_dataset: dict,
    ...
    for attr in [
        ...
        if attr_type == "number_field":
            ...
            map_data["results"]["per_point_data"][attr] = [
                len(v) for v in get_field_from_all_items(items_by_dataset, sorted_ids, attr_parameter, [])
            ]
        elif attr_type == "rank":
            ...
            decision_vector_data = get_trained_classifier(
                attr_parameter.collection_id,
                attr_parameter.class_name,
                embedding_space.identifier,
                include_vector=True,
            )
            ...
            vectors = np.asarray(
                get_field_from_all_items(
                    items_by_dataset, sorted_ids, attr_parameter.target_vector_field, dummy_vector
                )
            )
...
def get_map_selection_statistics(map_id: str, selected_ids: list[tuple[int, str]]) -> dict | None:
    ...
    cluster_id_per_point = [
        0 if any(ds_id == s[0] and item_id == s[1] for s in selected_ids) else 1 for ds_id, item_id in sorted_ids
    ]
    result_language = params.search.result_language
    cluster_data = get_cluster_titles(
        cluster_id_per_point, final_positions, sorted_ids, items_by_dataset, datasets, result_language, timings
