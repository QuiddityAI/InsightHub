...
def clusterize_results(projections, clusterizer_parameters: DotDict):
    min_cluster_size = (
        max(3, len(projections) // 50)
        if clusterizer_parameters.get("min_cluster_size", -1) <= 0
        else clusterizer_parameters.min_cluster_size
    )
...
def get_cluster_titles(
    ...
    for result_index, cluster_id in enumerate(cluster_id_per_point):
        ...
        text = join_text_source_fields(
            items_by_dataset[ds_id][item_id], descriptive_text_fields, field_boundary_indicator_padded
        )
    ...
    context_specific_tokenize = partial(
        tokenize, context_specific_ignore_words=dataset_specific_ignore_words, use_lower_case=use_lower_case
    )
    ...
    context_specific_tokenize = partial(
        tokenize, context_specific_ignore_words=all_ignore_words, use_lower_case=use_lower_case
    )
    vectorizer = TfidfVectorizer(
        tokenizer=context_specific_tokenize, lowercase=False, strip_accents=None, ngram_range=(1, 2), max_df=0.7
    )
    try:
        ...
        vectorizer = TfidfVectorizer(
            tokenizer=context_specific_tokenize, lowercase=False, strip_accents=None, ngram_range=(1, 2)
        )
        vectorizer.fit(texts_per_cluster)
    ...
    vectorizer = TfidfVectorizer(
        tokenizer=context_specific_tokenize,
        lowercase=False,
        strip_accents=None,
        ngram_range=(1, 2),
        max_df=0.7,
    ...
    for cluster_id in range(num_clusters):
        ...
        avg_score = np.mean(scores_per_cluster[cluster_id])
        cluster_title = (
            ", ".join(list(most_important_words))
            + f" ({len(point_positions_per_cluster[cluster_id])}x, {avg_score * 100:.0f}%)"
        )
        cluster_title_html = f'<span style="font-weight: bold;">{most_important_words[0]}</span>, '
        cluster_title_html += (
            ", ".join(list(most_important_words[1:]))
            + f" ({len(point_positions_per_cluster[cluster_id])}x, {avg_score * 100:.0f}%)"
        )
        cluster_data.append(
            {
                "id": cluster_id,
                "title": cluster_title,
                "title_html": cluster_title_html,
                "center": cluster_center,
                "min_score": min_score,
                "max_score": max_score,
                "avg_score": avg_score,
                "important_words": list(
                    zip(words[most_important_words_idx[:5]], word_weights[most_important_words_idx[:5]])
                ),
            }
        )
