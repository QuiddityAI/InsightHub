...
def get_search_results(params_str: str, purpose: str, timings: Timings | None = None) -> dict:
    ...
    for dataset_id in params.search.dataset_ids:
        ...
        if dataset.source_plugin == SourcePlugin.BING_WEB_API and params.search.search_type == "external_input":
            ...
            limit = (
                params.search.result_list_items_per_page
                if purpose == "list"
                else params.search.max_items_used_for_mapping
            )
            ...
            sorted_ids, full_items, total_matches = bing_web_search_formatted(
                dataset.id,
                query,
                limit=limit,
                offset=offset,
                website_filter=dataset.source_plugin_parameters.get("website_filter"),
            )
        ...
        if (
            dataset.source_plugin == SourcePlugin.SEMANTIC_SCHOLAR_API
            and params.search.search_type == "external_input"
        ):
            query = params.search.all_field_query
            limit = (
                params.search.result_list_items_per_page
        ...
        if params.search.search_type == "external_input":
            if params.search.use_separate_queries:
                sorted_ids, full_items = get_search_results_using_separate_queries(
                    dataset, params.search, params.vectorize, purpose, timings
                )
            else:
                sorted_ids, full_items, score_info, total_matches = get_search_results_using_combined_query(
                    dataset, params.search, params.vectorize, purpose, timings
                )
        elif params.search.search_type == "cluster":
            sorted_ids, full_items = get_search_results_for_cluster(
                dataset, params.search, params.vectorize, purpose, timings
            )
        elif params.search.search_type == "map_subset":
            sorted_ids, full_items = get_search_results_for_map_subset(
                dataset, params.search, params.vectorize, purpose, timings
            )
        elif params.search.search_type == "collection":
            sorted_ids, full_items = get_search_results_included_in_collection(
                dataset, params.search, params.vectorize, purpose, timings
            )
        elif params.search.search_type == "recommended_for_collection":
            sorted_ids, full_items, score_info = get_search_results_matching_a_collection(
                dataset, params.search, params.vectorize, purpose, timings
            )
        elif params.search.search_type == "similar_to_item":
            assert isinstance(similar_item_info, tuple)
            sorted_ids, full_items, score_info = get_search_results_similar_to_item(
                dataset, params.search, params.vectorize, purpose, timings, similar_item_info
            )
        elif params.search.search_type == "random_sample":
            sorted_ids, full_items, score_info, total_matches = get_search_results_for_global_map(
                dataset, params.search, params.vectorize, purpose, timings
            )
        else:
...
def get_search_results_using_combined_query(
    ...
    if search_settings.separate_queries:
        enabled_fields = [
            field_identifier
            for field_identifier, field_settings in search_settings.separate_queries.items()
            if field_settings["use_for_combined_search"]
        ]
    limit = (
    ...
    for query in queries:
        ...
        for field in dataset.schema.object_fields.values():
            ...
            if search_settings.retrieval_mode in ["vector", "hybrid"] and field.field_type == FieldType.VECTOR:
                score_threshold = get_field_similarity_threshold(
                    field, input_is_image=bool(query.positive_image_url or query.negative_image_url)
                )
                score_threshold = score_threshold if search_settings.use_similarity_thresholds else None
                results = get_vector_search_results(
                    dataset,
                    field.identifier,
                    query,
                    None,
                ...
                continue
        if text_fields:
            results, total_matches = get_fulltext_search_results(
                dataset,
                text_fields,
                query,
                filters,
    ...
    return (
        *combine_and_sort_result_sets(result_sets, required_fields, dataset, search_settings, limit, timings),
        actual_total_matches,
    )


def get_search_results_using_separate_queries(
    dataset, search_settings: DotDict, vectorize_settings: DotDict, purpose: str, timings: Timings
) -> tuple[list, dict]:
    return [], {}


def get_search_results_for_cluster(
    dataset, search_settings: DotDict, vectorize_settings: DotDict, purpose: str, timings: Timings
) -> tuple[list, dict]:
...
def get_search_results_for_map_subset(
    dataset, search_settings: DotDict, vectorize_settings: DotDict, purpose: str, timings: Timings
) -> tuple[list, dict]:
    origin_map_id: str = search_settings.cluster_origin_map_id
...
def _get_item_for_similarity_search(search_settings):
    ...
    vector_fields = [
        field
        for field in dataset.schema.object_fields.values()
        if field.identifier in dataset.schema.default_search_fields
        and field.field_type == FieldType.VECTOR
        and field.is_array == False
    ]
    search_engine_client = TextSearchEngineClient.get_instance()
    items = search_engine_client.get_items_by_ids(
        dataset, [item_id], fields=[field.identifier for field in vector_fields]
    )
...
def get_search_results_similar_to_item(
    dataset,
    search_settings: DotDict,
    vectorize_settings: DotDict,
    purpose: str,
    timings: Timings,
    similar_item_info: tuple,
    ...
    for field in vector_fields:
        ...
        if not [
            org_field
            for org_field in origin_vector_fields
            if org_field.identifier == field.identifier
            and org_field.actual_embedding_space.identifier == field.actual_embedding_space.identifier
        ]:
            continue
        ...
        results = get_vector_search_results(
            dataset,
            field.identifier,
            QueryInput(search_settings.all_field_query, search_settings.all_field_query_negative),
            query_vector,
            None,
            required_fields=[],
            internal_input_weight=search_settings.internal_input_weight,
        ...
        if origin_item["_dataset_id"] == dataset.id and origin_item["_id"] in results:
            ...
            max_score = max(
                [item["_origins"][0]["score"] for item in results.values() if item["_id"] != origin_item["_id"]]
            )
...
def get_search_results_matching_a_collection(
    dataset, search_settings: DotDict, vectorize_settings: DotDict, purpose: str, timings: Timings
) -> tuple[list, dict, dict]:
    ...
    dataset_specific_settings = next(
        filter(lambda item: item.dataset_id == dataset.id, collection.dataset_specific_settings), None
    )
    vector_field = None
    ...
    decision_vector_data = get_trained_classifier(
        collection_id, class_name, embedding_space_identifier, include_vector=True
    )
...
def get_search_results_included_in_collection(
    dataset, search_settings: DotDict, vectorize_settings: DotDict, purpose: str, timings: Timings
) -> tuple:
    ...
    for item_id in collection.positive_ids:
        total_items[item_id] = {
            ...
            "_origins": [{"type": "collection", "field": "positives", "query": "", "score": 1.0, "rank": 1}],
            "_score": 1.0,
    ...
    for item_id in collection.negative_ids:
        total_items[item_id] = {
            "_id": item_id,
            "_dataset_id": dataset.id,
            "_origins": [{"type": "collection", "field": "negatives", "query": "", "score": 0.0, "rank": 1}],
            "_score": 0.0,
...
def get_full_results_from_meta_info(
    dataset, vectorize_settings, search_result_meta_info: dict, purpose: str, timings
) -> tuple[list[str], dict[str, dict]]:
    required_fields = get_required_fields(dataset, vectorize_settings, purpose)
    return sort_items_and_complete_them(
        dataset, search_result_meta_info, required_fields, len(search_result_meta_info), timings
    )
...
def get_items_having_value_count(dataset_id: int, field: str, count_sub_items: bool = False) -> int:
    ...
    if dataset.schema.object_fields[field].field_type == FieldType.VECTOR:
        ...
        try:
            count = search_engine_client.get_item_count(dataset)
            return count - search_engine_client.get_all_items_with_missing_field_count(
                dataset.actual_database_name, field
            )
        except Exception as e:
            logging.error(e)
            return 0
