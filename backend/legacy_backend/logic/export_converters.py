...
def export_collection(collection_id: int, class_name: str, export_converter_identifier: str) -> dict:
    ...
    items, collection_entries = _get_exportable_collection_items(
        collection_id, class_name, converter_definition, required_fields
    )
...
def export_collection_table(
    collection_id: int, class_name: str, export_converter_identifier: str, format_identifier: str
) -> dict:
    converter_definition = get_export_converter(export_converter_identifier)
    converter = get_export_converter_module(converter_definition.module, converter_definition.parameters)
    required_fields = converter.required_fields()
...
def _get_exportable_collection_items(
    collection_id: int, class_name: str, converter_definition: DotDict, required_fields: tuple[str]
) -> tuple[list, list]:
    collection_items = get_collection_items(
        collection_id, class_name, field_type=FieldType.IDENTIFIER, is_positive=True, include_column_data=True
    )
    ...
    for ds_id in collection_items_per_dataset:
        ...
        if not converter_definition.universally_applicable and not any(
            converter
            for converter in dataset.schema.applicable_export_converters
...
class TableExportFormat:
    def export(self, collection: DotDict, items: list, collection_entries: list, converter: ExportConverter) -> dict:
        raise NotImplementedError

    def _get_rows(
        self, collection: DotDict, items: list, collection_entries: list, converter: ExportConverter
    ) -> tuple[list, list]:
