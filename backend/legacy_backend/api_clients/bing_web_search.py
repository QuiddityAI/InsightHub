...
def bing_web_search_formatted(dataset_id: int, query: str, website_filter: str | None = None, limit: int = 300, offset: int = 0):
    ...
    for i, item in enumerate(data):
        ...
        item["_origins"] = [{'type': 'web_search', 'field': 'unknown',
                          'query': query, 'score': score, 'rank': offset+i+1}]
        item["_score"] = score
...
def bing_web_search(query: str, website_filter: str | None = None, limit: int = 50, offset: int = 0) -> tuple[list, int]:
    ...
    while len(results) < limit:
        ...
        offset += len(results)
        partial_results, total_matches = bing_web_search_call(query, website_filter, min(per_page, limit - len(results)), offset)
