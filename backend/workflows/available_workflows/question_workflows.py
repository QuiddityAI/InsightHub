...
class FindFactFromSingleDocumentWorkflow(WorkflowBase):
    metadata: WorkflowMetadata = WorkflowMetadata(
        ...
        name2={'en': "Fact in a <entity_name_singular>", 'de': "Fakt in <entity_name_plural>"},
        help_text={'en': "For questions that can be answered based on one item", 'de': "Für Fragen, die anhand eines Dokuments beantwortet werden können"},
...
class CollectFactsFromMultipleDocumentsWorkflow(FindFactFromSingleDocumentWorkflow):
    metadata: WorkflowMetadata = WorkflowMetadata(
        ...
        name2={'en': "From Multiple <entity_name_plural>", 'de': "Aus mehreren <entity_name_plural>"},
        help_text={'en': "Collect information found in multiple documents", 'de': "Sammle Informationen aus mehreren Dokumenten"},
        query_field_hint={'en': "Your question", 'de': "Deine Frage"},
        supports_filters=True,
        needs_user_input=True,
        needs_result_language=True,
...
class BeyondExistingFactsWorkflow(FindFactFromSingleDocumentWorkflow):
    metadata: WorkflowMetadata = WorkflowMetadata(
        ...
        help_text={'en': "E.g. analyze the status and history of a project", 'de': "Z.B. den Status und die Geschichte eines Projekts analysieren"},
        query_field_hint={'en': "Your question", 'de': "Deine Frage"},
        supports_filters=True,
        needs_user_input=True,
        needs_result_language=True,
